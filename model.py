...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    for i in range(max_len):
        if i < len(ids):
            paded_ids.append(ids[i])
        else:
            paded_ids.append(pad_id)
...
def build_sinusoidal_positional_encoding(max_len, d_model):
    ...
    # Compute divisor term and position_indices with helper functions
    div_term = compute_positional_div_term(d_model)
    position_indices = build_position_index_column(max_len)

    # fil sin and cos columns 
    pe = fill_even_indices_with_sin(pe,position_indices,div_term)
    pe = fill_odd_indices_with_cos(pe,position_indices,div_term)
...
def add_positional_encoding_to_embeddings(embedded_batch, positional_encoding):
    ...
    sequence_length = embedded_batch.shape[1]
    return embedded_batch + positional_encoding[0:sequence_length]
...
def build_padding_mask(token_ids, pad_id):
    ...
    mask = (token_ids != pad_id).unsqueeze(1).unsqueeze(1)
    return mask
...
def build_causal_mask(seq_len):
    ...
    tensor_ones = torch.ones(seq_len, seq_len, dtype=torch.bool)
    mask = torch.tril(tensor_ones)
...
def softmax_attention_weights(masked_scores):
    ...
    Rows consisting entirely of -inf are returned as zeros.
    """
    all_masked = torch.all(
        masked_scores == float("-inf"),
        dim=-1,
        keepdim=True
    )
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    # unpacked 
    multi_head_tensor = multi_head_tensor.transpose(-3,-2).contiguous()
    B,L,num_heads,d_k = multi_head_tensor.shape
    # do operation 
